...
4. Fill in the block:
   ...
   - Each scenario = one blockquoted block (`> **Scenario: <behavior>_<outcome>**`, with `>` on every line) with a single When; the name becomes the test function name.
...
8. Append the block to the module's Requirements section as a level-3 heading (`### REQ-ID - Short title`), nested under `## Requirements`; if the REQ introduces or changes a route, update the "Exposed routes" table in the same file.
